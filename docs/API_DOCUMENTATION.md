...
**工作流节点说明**:
1. **情报分析**: 从灾情描述中提取结构化信息（类型、等级、位置、受影响人数等）
2. **RAG 检索**: 根据提取的灾情信息，从向量数据库检索相关应急预案文档
3. **资源调度**: 根据灾情类型和等级，从 MySQL 数据库查询可用应急资源
4. **方案生成**: 结合情报分析结果、RAG 检索结果和资源调度结果，生成完整应急处置方案
5. **方案审查**: 审查方案合规性，给出评分和修改建议
...
| 资源调度 Agent | 从 MySQL 数据库查询可用应急资源（人员、设备、物资） | pymysql |
| 方案生成 Agent | 根据情报分析结果、RAG 检索结果和资源调度结果生成完整应急处置方案 | vLLM completions API |
...
```json
{
    "plan": "### 云南省地震灾害应急处置方案\n\n#### 1. 事件概况\n- **灾害类型**: 地震\n- **灾害等级**: 中\n- **发生时间**: 未知\n- **发生地点**: 云南省昆明市五华区\n- **受影响人数**: 500\n- **置信度**: 0.8\n\n#### 2. 风险评估\n...（后续内容）\n\n#### 4. 资源调度建议\n- **急救药品**: 调度昆明市急救中心库存 800 箱\n- **消毒用品**: 调度云南省疾控中心库存 400 箱\n- **救灾帐篷**: 调度云南省救灾物资储备中心库存 300 顶\n- **森林消防救援队**: 调度云南省森林消防总队 250 人\n- **地震救援队**: 调度昆明市五华区救援队 150 人\n\n#### 5. 应急措施\n...（后续内容）\n\n### 参考资料\n1. 云南省地震应急预案\n   - 章节: 4\n   - 相似度: 0.6275\n   - 内容: 各级政府及其有关部门应对本行政区域内地震灾害风险进行评估...\n2. 昆明市自然灾害应急预案\n   - 章节: 8\n   - 相似度: 0.6124\n   - 内容: 边界地区政府要加强与相邻地区的信息沟通...\n3. 五华区应急处置手册\n   - 章节: 6\n   - 相似度: 0.6029\n   - 内容: 城乡社区应做好受灾群众的临时安置工作..."
}
```
...
**响应时间**: 约 25-125 秒（包含 RAG 检索和资源调度时间，取决于重试次数）
...
**响应时间**: 约 25-125 秒（首次响应时间取决于工作流执行完成时间，包含 RAG 检索和资源调度）
...
| 指标 | 数值 | 说明 |
|------|------|------|
| 首次推理延迟（情报分析） | ~1.66 秒 | 从请求到第一个 token 的时间 |
| 后续推理延迟（方案生成） | ~0.8 秒 | 模型已预热后的延迟 |

### 3.2 单 Agent 推理耗时（含资源调度）

| Agent | 单次耗时 | 范围 |
|-------|----------|------|
| 情报分析 Agent | 1.66 秒 | 0.8 ~ 3.8 秒 |
| RAG 检索模块 | 9.13 秒 | 8 ~ 12 秒（含模型加载和数据库查询） |
| 资源调度 Agent | **0.01 秒** | 0.01 ~ 0.1 秒（MySQL 数据库查询，极快） |
| 方案生成 Agent | 11.97 秒 | 10 ~ 13 秒 |
| 方案审查 Agent | 2.80 秒 | 2.5 ~ 8.7 秒 |
| **单次流程总耗时** | **25.57 秒** | - |
| **含重试完整耗时** | **122.55 秒** | 取决于审查评分和重试次数 |
