...
/**
 * 【新增】：轻量级 Markdown 解析渲染函数，将 ###、** 及列表转化为规范 HTML
 */
function renderMarkdown(content: string): string {
  ...
  // 2. 【核心修复】：防止前后文本粘连，强制在 # 号标题前增加换行，并完美解析各级标题
  html = html.replace(/([^\n])\s*(#{1,6})\s*/g, '$1\n$2 ')

  html = html.replace(/#{1,6}\s*([^\n<]+)/g, (_match, titleText) => {
    const cleanText = titleText.replace(/^#+/, '').trim()
    return `<div style="margin: 16px 0 8px 0; font-weight: bold; font-size: 15px; color: #1a73e8; border-left: 3px solid #1a73e8; padding-left: 8px;">${cleanText}</div>`
  })
  ...
  // 4. 转换列表项 (- 格式)
  html = html.replace(/^\s*-\s+(.*)$/gm, '<li style="margin-left: 20px; list-style-type: disc; line-height: 1.8;">$1</li>')
    ...
    <el-card shadow="hover" v-loading="incidentStore.loading">
      <template v-if="incidentStore.currentIncident">
        ...
        >
          ...
          <!-- 【修改这里】：使用 v-html 配合 renderMarkdown 函数，规范渲染标题、加粗和列表 -->
