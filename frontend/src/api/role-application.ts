...
export function submitRoleApplication(data: RoleApplicationSubmitData & { userId?: number }): Promise<ApiResponse<RoleApplication>> {
...
export function receiveRoleApplication(data: { id: number }): Promise<ApiResponse<RoleApplication>> {
  return request.post('/role-application/receive', data)
}