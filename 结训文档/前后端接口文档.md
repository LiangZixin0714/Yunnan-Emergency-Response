...
| X-API-Key | emergency-platform-ai-service-key-2024 | 是 |
...
**请求方法**：POST  
**URL**：`http://localhost:8002/api/v1/knowledge/vectorize`  
**需要认证**：是（X-API-Key）
...
| 字段 | 类型 | 必填 | 说明 |
|------|------|------|------|
| objectKey | String | 是 | MinIO对象键（文件路径） |
| bucket | String | 是 | MinIO桶名 |
| fileId | String | 是 | 文件唯一标识 |
| fileName | String | 是 | 文件名 |
| chunkSize | Integer | 否 | 切片大小，默认800字符 |
| chunkOverlap | Integer | 否 | 切片重叠，默认100字符 |
...
**响应时间**：约30-60秒/文档（取决于切片数量）

### 10.5 向量删除

**请求方法**：POST  
**URL**：`http://localhost:8002/api/v1/knowledge/vectors/delete`  
**需要认证**：是（X-API-Key）
...
| MYSQL_HOST | localhost | MySQL主机（资源调度） |
| MYSQL_PORT | 3306 | MySQL端口 |
| MYSQL_DATABASE | emergency_db | MySQL数据库名 |
| BACKEND_URL | http://localhost:8080 | 后端服务地址 |
| AI_SERVICE_API_KEY | - | API Key认证密钥 |
| VLLM_BASE_URL | http://127.0.0.1:8000/v1 | vLLM推理服务地址 |
...
| V4.0 | 2026-07-28 | 系统生成 | 整合AI服务说明文档内容：补充AI服务接口鉴权方式（X-API-Key），补充知识库向量化接口（POST /api/v1/knowledge/vectorize），补充向量删除接口（POST /api/v1/knowledge/vectors/delete），补充AI服务环境变量配置，补充incidentId字段到方案生成请求 |
